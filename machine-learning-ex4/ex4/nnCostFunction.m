...
cost = (1.0 /m)* sum(sum((-y_nn.* log(Q)) - (1 - y_nn).* log(1 - Q)));
p = Theta1.^2;
g = Theta2.^2;
c = sum(sum(p(:, 2:size(p ,2))));
d = sum(sum(g(:, 2:size(g ,2))));


reg_term = (lambda/ (2*m))* (c + d);
 
J = cost + reg_term; 

%cd 'C:\Users\BARUWA1\Desktop\AI & ML\andrew ng\machine-learning-ex4/ex4'
